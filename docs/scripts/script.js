...
if (localStorage.getItem("safeMode") === "true") {
  Object.keys(PAGE).forEach((pageKey) => {
    if (!UNSAFE_PAGES.includes(pageKey)) {
      ...
      page.content = removeContentMarkers(page.content, "unsafe");
    } else {
      delete PAGE[pageKey];
    }
  });
  ...
  Object.keys(PAGE).forEach((pageKey) => {
      page = PAGE[pageKey];
      page.content = removeContentMarkers(page.content, "safe");
  });
}
...
function generateSpeedrun(setRun, setSpeedrunLength) {
  ...
  let speedrunPath = [];
    ...
    if (reverseSpeedrun) { speedrunPath = speedrunPath.toReversed(); }
    return speedrunPath;
}
...
function wikifyText(text) {
  ...
  for (file in fileList) {
    if (fileList[file]) {
      if (fileList[file].includes("seealso>>")) {
        ...
        seeAlsoList.push.apply(seeAlsoList, fileFull[0].split("|"));
      }
    }
  }
  ...
  if (seeAlsoText != defaultSeeAlsoText) {
    completeText += wikifyText(seeAlsoText + ";;</div>");
  }
